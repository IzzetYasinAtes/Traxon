...
public sealed class PaperPolymarketEngine : ITradingEngine
{
    ...
    /// Gamma API'den ResolvedPrice gelene kadar bekler — LivePoly ile birebir aynı mantık.
    /// </summary>
        ...
        foreach (var (tradeId, trade) in _openTrades)
        {
            try
            {
                ...
                if ((DateTime.UtcNow - trade.OpenedAt) > TimeSpan.FromMinutes(30))
                {
                    ...
                    _logger.LogWarning(
                        "[PaperPoly] Trade TIMEOUT (>30min): {Asset} {Direction} opened {Opened}",
                ...
                _logger.LogInformation(
                    "[PaperPoly] Market RESOLVED via GammaAPI for {Asset} {Direction}: ResolvedPrice={Price}",
